...
class TaskTimer:
    ...
    def cycle_task(self, task):
        ...
        if task['next_sec'] <= int(time.time()):
            try:
                ...
                self.write_log("异常", "周期任务：" +
                               task['fun'].__name__ + " 函数内部异常：" + str(e))
            finally:
    ...
    def timing_task(self, task):
        ...
        if not task['today_done']:
            if today_sec >= task['task_sec']:  # 到点了，开始执行任务
                try:
                    task['fun'](*task['arg'])
                    self.write_log("正常", "定时任务：" +
                                   task['fun'].__name__ + " 已执行")
                except Exception as e:
                    self.write_log("异常", "定时任务：" +
                                   task['fun'].__name__ + " 函数内部异常：" + str(e))
                finally:
                    task['today_done'] = True
                    if task['first_work']:
    ...
    def join_task(self, fun, arg, interval=None, timing=None, immediately=False):
        """
    ...
    def stop(self):
        util._terminateThread(self.thread)
